...
struct _xmpp_helper_conn_t {
    unsigned int ref;
    xmpp_ctx_t *ctx;
    xmpp_conn_type_t type;
    int is_raw;

    xmpp_conn_type_t state;
...
xmpp_t *xmpphelper_new(xmppconn_handler cb, xmpp_mem_t *mem, xmpp_log_t *log, void *userdata)
{
    ...
    //This is for libstrophe-0.9.2 to ignore openfire self-signed certification
    //xmpp_conn_set_flags(xmpp->conn, XMPP_CONN_FLAG_TRUST_TLS);
    xmpp->force_tls = 0;
    xmpp->callback = cb;
...
int xmpphelper_connect(xmpp_t *xmpp, char *host, int port, char *jid, char *pass)
{
    ...
    xmpp_conn_set_keepalive(xmpp->conn, 10, 30);
    //xmpp_conn_disable_tls(xmpp->conn);

    rc = xmpp_connect_client(xmpp->conn, host, port, _conn_handler, xmpp);

    //int kacnt = 1;
    //rc = setsockopt(xmpphelper_get_sock(xmpp), IPPROTO_TCP, TCP_KEEPCNT, (char *) &kacnt, sizeof(kacnt));
    return rc;
}
...
int xmpphelper_get_sock(xmpp_t *xmpp)
{
    struct _xmpp_helper_conn_t * conn;
    conn = (struct _xmpp_helper_conn_t *) xmpp->conn;
    return conn->sock;
}
